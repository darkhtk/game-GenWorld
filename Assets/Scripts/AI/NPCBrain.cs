...
public class NPCBrain
{
    ...
    public void Restore(NPCBrainData data) { if (data == null) return; CurrentMood = Enum.TryParse<Mood>(data.mood, true, out var m) ? m : Mood.Neutral; _relationships.Clear(); if (data.relationships != null) foreach (var k in data.relationships) _relationships[k.Key] = k.Value; _memories.Clear(); if (data.memories != null) _memories.AddRange(data.memories); _triggeredEvents.Clear(); if (data.triggeredEvents != null) foreach (var t in data.triggeredEvents) _triggeredEvents.Add(t); WantToTalk = data.wantToTalk; TalkReason = data.talkReason ?? ""; AlertType = data.alertType ?? ""; }
}
