...
| ~~S-123~~ | ~~🎨 인벤토리 빈 슬롯 그래픽 톤다운 (alpha 0.3)~~ 👀 2026-04-30                       | P3   | UI      | In Review (Supervisor 8회차) — `GameConfig.UI.InventorySlotEmptyAlpha=0.3f` / `InventorySlotFilledAlpha=1.0f` + `InventorySlotUI.ApplyEmptyAlpha(bool)` 헬퍼 + Clear()/SetItem() 훅. 자산 신규 0(slot_bg 알파만 조정). EditMode `InventorySlotEmptyAlphaTests` 6건. |
