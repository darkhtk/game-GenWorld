...
| ~~S-140~~ | ~~🎨 보스 처치 시 화면 흔들림 + 승리 코드 SFX~~ 👀 2026-04-30 | P2   | VFX/SFX | In Review (Supervisor 9회차) — 자산 신규 0(기존 `sfx_boss_defeat.wav` ~1.5s orphan asset 재사용, S-122 `sfx_click` 재사용 선례). `GameConfig.Audio` S-140 7상수 추가(BossDeathSfxName/Volume=0.95/CameraShakeMs=600/Intensity=0.022/DuckBgmDropDb=-8/Duration=1.2/Enabled). `CombatRewardHandler.OnMonsterKilled` 보스 분기(`def.rank == "boss"` && Enabled) → CameraShake.Shake(_host, 600ms, 0.022) + AudioManager.PlaySFXScaled + DuckBGM(-8dB, 1.2s, S-118 -6dB/0.4s 보다 깊고 김). EditMode `BossDeathConfigTests.cs` 7건. |
